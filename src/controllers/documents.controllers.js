...
const getDocumentByFilters1 = async (req, res, next) => {
    try {
        const tenant = req.params.tenant;
        let { filters, page, itemsPerPage} = req.query;
...
const createDocument = async (req, res, next) => {
    try {
        ...
        const { id_venta, fecha_de_emision, codigo_tipo_documento, serie_documento, 
        ...
        const response = await pool.query(
            ...
            [ now, now, fecha_de_emision, id_venta, codigo_tipo_documento, serie_documento, 
                ...
                totales.total_venta, 'N', JSON.stringify(strdocument), company ]);
