...
const selectApiCompanyById = async (id) => {
    try {
        ...
        const company = await pool.query(`SELECT url, token, tenant, autosend, token_series, external_api, company_number, company, address FROM public.company WHERE state=true AND id_company = $1`, [id]);
...
const selectAllApiCompany = async () => {
    try {
        const company = await pool.query(`SELECT id_company, company_number, company, url, token, tenant, autosend, localtoken, state FROM public.company WHERE state=true ORDER BY company`);
        if (!company.rowCount) { return false; }
        return company.rows
