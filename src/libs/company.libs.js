...
const selectApiCompanyById = async (id) => {
    try {
        ...
        const company = await pool.query(`SELECT url, token, tenant, autosend, token_series, external_api, company_number FROM public.company WHERE state=true AND id_company = $1`, [id]);
