...
export const documentsCommand = new Command('documents')
  ...
  .action(async (options) => {
    ...
    try {
      ...
      let tenantsToQuery: string[] = [];
      
      if (options.tenant) {
        tenantsToQuery.push(options.tenant);
      } else {
        const companies = await pool.query('SELECT tenant FROM public.company WHERE tenant IS NOT NULL');
      ...
      if (options.json) {
        ...
        if (options.tenant) md += `**Empresa:** ${options.tenant}\n`;
        if (options.status) md += `**Estado Filtrado:** ${options.status}\n`;
        ...
        if (options.status && !options.tenant) {
          ...
          md += `| Empresa | Cantidad |\n`;
          md += `|---------|----------|\n`;
          const summaryArray = Object.entries(summary).map(([empresa, cantidad]) => ({ Empresa: empresa, Cantidad: cantidad as number }));
        ...
        if (options.status && !options.tenant) {
          ...
          summaryArray.sort((a, b) => (b.Cantidad as number) - (a.Cantidad as number));
